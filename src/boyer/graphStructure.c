...
void _FillVisitedFlagsInOtherBicomps(graphP theGraph, int BicompRoot,
                                     int FillValue);
...
int gp_InitGraph(graphP theGraph, int N) {
  ...
  if ((theGraph->G = (graphNodeP)malloc((2 + 2 * EDGE_LIMIT) * N *
                                        sizeof(graphNode))) == NULL ||
      ...
      (theGraph->extFace =
           (extFaceLinkRecP)malloc(2 * N * sizeof(extFaceLinkRec))) == NULL ||
      0) {
    _ClearGraph(theGraph);
    return NOTOK;
  }
...
void _InitGraphNode(graphP theGraph, int I) {
  theGraph->G[I].v = theGraph->G[I].link[0] = theGraph->G[I].link[1] = NIL;
...
void _InitVertexRec(graphP theGraph, int I) {
  theGraph->V[I].leastAncestor = theGraph->V[I].Lowpoint = I;
...
void _ClearIsolatorContext(graphP theGraph) {
  ...
  IC->minorType = 0;
  IC->v = IC->r = IC->x = IC->y = IC->w = IC->px = IC->py = IC->z = IC->ux =
      IC->dx = IC->uy = IC->dy = IC->dw = IC->uz = IC->dz = NIL;
}
...
int _getUnprocessedChild(graphP theGraph, int parent) {
  ...
  if (theGraph->G[J].link[0] != theGraph->G[J].link[1]) {
    theGraph->G[theGraph->G[JTwin].link[0]].link[1] =
        theGraph->G[JTwin].link[1];
    theGraph->G[theGraph->G[JTwin].link[1]].link[0] =
        theGraph->G[JTwin].link[0];
...
static void _insert_arc(graphP theGraph, int edge, int arc, int link) {
  int u0 = theGraph->G[edge].link[link];
  theGraph->G[arc].link[link] = u0;
  theGraph->G[arc].link[1 ^ link] = edge;
  theGraph->G[edge].link[link] = arc;
  theGraph->G[u0].link[1 ^ link] = arc;
}

// uedge - edge coming out of u.
// vedge - edge coming out of v.
// ulink - where to insert arc relative to edge from u.
// vlink - where to insert arc relative to edge from v.
int gp_InsertArc(graphP theGraph, int u, int v, int uedge, int vedge, int ulink,
  ...
  theGraph->G[vpos].v = v;
  theGraph->G[upos].v = u;
  _insert_arc(theGraph, uedge, vpos, ulink);
  _insert_arc(theGraph, vedge, upos, vlink);
